...
static inline void e1000_write_reg(e1000_device_t* dev, uint32_t offset, uint32_t val) {
        ...
        if (length > sizeof(ethernet_header_t)) {
            ...
            switch (ether_type) {
                case ARP_ETHER_TYPE:
                    ...
                    arp_handle_packet(packet_buffer + sizeof(ethernet_header_t), length - sizeof(ethernet_header_t));
                    break;
                case 0x0800: // ETHERTYPE_IPV4
                    klog(LOG_INFO, "E1000: Received IPv4 packet.");
                    ip_handle_packet(network_devices, packet_buffer + sizeof(ethernet_header_t), length - sizeof(ethernet_header_t));
                    break;
                default:
...
// --- E1000 Send Packet ---
static void e1000_send_packet(net_dev_t* net_dev, const uint8_t* buffer, size_t size) {
    ...
    while (!(desc->status & 0x01)) { // DD bit
        // For now, busy wait. Future: yield or block thread
    }
    ...
    // Advance TDT
    e1000_write_reg(e1000_dev, E1000_REG_TDT, (current_tdt + 1) % E1000_NUM_TX_DESC);
...
static int e1000_attach(device_t* dev) {
    ...
    uint16_t pci_command_reg = pci_read_config_word(dev->bus, dev->device, dev->func, PCI_COMMAND);
    e1000_dev->pci_command_reg = pci_command_reg; // Save original
    pci_command_reg |= (1 << 2); // Set Bus Master Enable bit
    pci_write_config_word(dev->bus, dev->device, dev->func, PCI_COMMAND, pci_command_reg);

    // 2. Map MMIO registers (assuming identity mapped for now)
    uint32_t bar0 = pci_read_config_dword(dev->bus, dev->device, dev->func, 0x10);
    uint32_t mmio_phys_base = bar0 & 0xFFFFFFF0;
    e1000_dev->mmio_base = (volatile uint32_t*) (uint64_t)mmio_phys_base; 
    ...
    klog(LOG_INFO, "E1000: MAC Address: XX:XX:XX:XX:XX:XX"); // Needs proper format printing

    // 4. Disable interrupts
    e1000_write_reg(e1000_dev, E1000_REG_IMC, 0xFFFFFFFF);

    // 5. Reset the device
    e1000_write_reg(e1000_dev, E1000_REG_CTRL, e1000_read_reg(e1000_dev, E1000_REG_CTRL) | (1 << 26));
    while (e1000_read_reg(e1000_dev, E1000_REG_CTRL) & (1 << 26));

    // 6. Setup Receive and Transmit Descriptor Lists
    // Allocate memory for Rx Descriptors
    e1000_dev->rx_descs = (struct e1000_rx_desc*)pmm_alloc_page();
    if (!e1000_dev->rx_descs) { klog(LOG_ERROR, "E1000: Failed to allocate Rx descriptor memory."); return -1; }
    e1000_dev->rx_descs_phys = (uint64_t)e1000_dev->rx_descs;
    memset(e1000_dev->rx_descs, 0, E1000_NUM_RX_DESC * sizeof(struct e1000_rx_desc));

    for (int i = 0; i < E1000_NUM_RX_DESC; i++) {
        e1000_dev->rx_buffers[i] = (uint8_t*)pmm_alloc_page();
        if (!e1000_dev->rx_buffers[i]) { klog(LOG_ERROR, "E1000: Failed to allocate Rx buffer."); return -1; }
    ...
    e1000_dev->tx_descs = (struct e1000_tx_desc*)pmm_alloc_page();
    if (!e1000_dev->tx_descs) { klog(LOG_ERROR, "E1000: Failed to allocate Tx descriptor memory."); return -1; }
    e1000_dev->tx_descs_phys = (uint64_t)e1000_dev->tx_descs;
    memset(e1000_dev->tx_descs, 0, E1000_NUM_TX_DESC * sizeof(struct e1000_tx_desc));

    for (int i = 0; i < E1000_NUM_TX_DESC; i++) {
        e1000_dev->tx_buffers[i] = (uint8_t*)pmm_alloc_page();
        if (!e1000_dev->tx_buffers[i]) { klog(LOG_ERROR, "E1000: Failed to allocate Tx buffer."); return -1; }
        e1000_dev->tx_descs[i].addr = (uint64_t)e1000_dev->tx_buffers[i];
        e1000_dev->tx_descs[i].cmd = E1000_TXD_CMD_EOP | E1000_TXD_CMD_RS; // Default commands
        e1000_dev->tx_descs[i].status = 0x01; // Set DD bit initially
    }
    e1000_dev->tx_cur = 0;

    // 7. Program the descriptor base addresses and lengths
    e1000_write_reg(e1000_dev, E1000_REG_RDBAL, (uint32_t)e1000_dev->rx_descs_phys);
    e1000_write_reg(e1000_dev, E1000_REG_RDBAL + 4, (uint32_t)(e1000_dev->rx_descs_phys >> 32));
    e1000_write_reg(e1000_dev, E1000_REG_RDLEN, E1000_NUM_RX_DESC * sizeof(struct e1000_rx_desc));
    e1000_write_reg(e1000_dev, E1000_REG_RDH, 0);
    e1000_write_reg(e1000_dev, E1000_REG_RDT, E1000_NUM_RX_DESC - 1);

    e1000_write_reg(e1000_dev, E1000_REG_TDBAL, (uint32_t)e1000_dev->tx_descs_phys);
    e1000_write_reg(e1000_dev, E1000_REG_TDBAL + 4, (uint32_t)(e1000_dev->tx_descs_phys >> 32));
    e1000_write_reg(e1000_dev, E1000_REG_TDLEN, E1000_NUM_TX_DESC * sizeof(struct e1000_tx_desc));
    e1000_write_reg(e1000_dev, E1000_REG_TDH, 0);
    e1000_write_reg(e1000_dev, E1000_REG_TDT, 0);

    // 8. Enable Receive and Transmit
    e1000_write_reg(e1000_dev, E1000_REG_RCTL, E1000_RCTL_EN | E1000_RCTL_BAM | E1000_RCTL_SBP | E1000_RCTL_BSIZE_2048);
    e1000_write_reg(e1000_dev, E1000_REG_TCTL, E1000_TCTL_EN | E1000_TCTL_PSP | (0x10 << 4) | (0x40 << 12));

    // 9. Register interrupt handler
    e1000_dev->irq = pci_read_config_byte(dev->bus, dev->device, dev->func, PCI_INTERRUPT_LINE);
    register_irq_handler(e1000_dev->irq, e1000_interrupt_handler);
    
    // 10. Enable interrupts
    e1000_write_reg(e1000_dev, E1000_REG_IMS, 0x1F6DC); // Enable RxDMT, RxO, RxUT, TxDW, TxQE, LSC, SDC, PHY

    // Register with network core
    e1000_dev->net_dev.send_packet = e1000_send_packet;
    e1000_dev->net_dev.receive_packet = NULL; // Zero-copy, so packets are handled in interrupt
